...
const AboutGurudevResolution = ({ BASE_URL }) => {
  // ✅ Table data
...
const apiUrl = `${BASE_URL}/api/insertabout_gurudevresolution`;
    console.log('Making request to:', apiUrl); // Log the URL
    if (editingId) {
      // ✅ Update existing
      const res = await axios.put(
        `${BASE_URL}/api/updateabout_gurudevresolution/${editingId}`,
        fd,
      ...
      // ✅ Add new
      const res = await axios.post(`${BASE_URL}/api/insertabout_gurudevresolution`, fd, {
        headers: { apikey: "12345", "Content-Type": "multipart/form-data" },
      });
...
//select all data table
 ...
 const [eventsslide, setEventsslide] = useState([]);
  ...
  // ✅ Fetch data from API
  useEffect(() => {
    axios.get(`${BASE_URL}/api/selectabout_gurudevresolution`, {
      headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/api/deleteabout_gurudevresolution/${id}`, {
  ...
  const toggleStatus = async (id) => {
    try {
      const res = await axios.patch(
        `${BASE_URL}/api/statusabout_gurudevresolution/${id}/status`,
        {},
        { headers: { apikey: "12345" } }
      );
  ...
  return (
    <div className="top-banner">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h3>About Gurudev Resolution</h3>
        <button
          className="btn btn-success btn-sm"
          onClick={() => setFormData(!formData)}
        >
          <i className="fa fa-plus"></i> {formData ? "Close Form" : "Add About Gurudev Resolution"}
        </button>
      </div>
          ...
          <div className="modal-dialog modal-lg" style={{ maxWidth: "1200px" }}>
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Edit Sant Marg Dershan</h5>
                <button
                  type="button"
                  className="btn-close"
...
export default AboutGurudevResolution;
