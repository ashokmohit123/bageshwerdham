...
const SantMargDershan = ({ BASE_URL }) => {
  // ✅ Table data
...
const apiUrl = `${BASE_URL}/api/insertabout_saintsdarshan`;
    console.log('Making request to:', apiUrl); // Log the URL
    if (editingId) {
      // ✅ Update existing
      const res = await axios.put(
        `${BASE_URL}/api/updateabout_saintsdarshan/${editingId}`,
        fd,
      ...
      // ✅ Add new
      const res = await axios.post(`${BASE_URL}/api/insertabout_saintsdarshan`, fd, {
        headers: { apikey: "12345", "Content-Type": "multipart/form-data" },
      });
...
//select all data table
 ...
 const [eventsslide, setEventsslide] = useState([]);
  ...
  // ✅ Fetch data from API
  useEffect(() => {
    axios.get(`${BASE_URL}/api/selectabout_saintsdarshan`, {
      headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/api/deleteabout_saintsdarshan/${id}`, {
  ...
  const toggleStatus = async (id) => {
    try {
      const res = await axios.patch(
        `${BASE_URL}/api/statusabout_saintsdarshan/${id}/status`,
        {},
        { headers: { apikey: "12345" } }
      );
  ...
  return (
    <div className="top-banner">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h3>About Sant Marg Dershan</h3>
        <button
          className="btn btn-success btn-sm"
          onClick={() => setFormData(!formData)}
        >
          <i className="fa fa-plus"></i> {formData ? "Close Form" : "Add About Sant Marg Dershan"}
        </button>
      </div>
