...
const KanyaVivahVideoSlider = ({ BASE_URL }) => {
  ...
  try {
    ...
    if (editingId) {
      const res = await axios.put(
        `${BASE_URL}/api/updatekanyavivah_videoslider/${editingId}`,
        fd,
        {
      ...
      const res = await axios.post(
        `${BASE_URL}/api/insertkanyavivah_videoslider`,
        fd,
        { headers: { apikey: "12345", "Content-Type": "multipart/form-data" } }
      );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/api/deletekanyavivah_videoslider/${id}`, {
  ...
  const toggleStatus = async (id) => {
    try {
      const res = await axios.patch(
        `${BASE_URL}/api/statuskanyavivah_videoslider/${id}/status`,
        {},
        { headers: { apikey: "12345" } }
      );
      setEventsslide(prev =>
  ...
  return (
    <div className="top-banner">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h3>Kanya Vivah Video Slider</h3>
        <button
          className="btn btn-success btn-sm"
          onClick={() => setShowForm(!showForm)}
        >
          <i className="fa fa-plus"></i> {showForm ? "Close Form" : "Add Kanya Vivah Video"}
        </button>
      </div>
      ...
      {/* ✅ Table */}
      <div className="card">
        <div className="card-header">Bageshwer Kanya Vivah Video Slider</div>
        <div className="card-body">
          ...
          <div className="modal-dialog modal-lg">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Edit Kanya Vivah Video Slider</h5>
