...
const UpcommingServiceCommitments = ({ BASE_URL }) => {
  // ✅ Table data
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    fd.append("icon_text", formData.icon_text);
...
const apiUrl = `${BASE_URL}/api/insertupcomming_servicecommitments`;
    console.log('Making request to:', apiUrl); // Log the URL
    if (editingId) {
      // ✅ Update existing
      const res = await axios.put(
        `${BASE_URL}/api/updateupcomming_servicecommitments/${editingId}`,
        fd,
      ...
      // ✅ Add new
      const res = await axios.post(`${BASE_URL}/api/insertupcomming_servicecommitments`, fd, {
        headers: { apikey: "12345", "Content-Type": "multipart/form-data" },
      });
...
//select all data table
 ...
 const [eventsslide, setEventsslide] = useState([]);
  ...
  // ✅ Fetch data from API
  useEffect(() => {
    axios.get(`${BASE_URL}/api/selectupcomming_servicecommitments`, {
      headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/api/deleteupcomming_servicecommitments/${id}`, {
  ...
  const toggleStatus = async (id) => {
    try {
      const res = await axios.patch(
        `${BASE_URL}/api/statusupcomming_servicecommitments/${id}/status`,
        {},
        { headers: { apikey: "12345" } }
      );
  ...
  return (
    <div className="top-banner">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h3>Upcoming Service Commitments</h3>
        <button
          className="btn btn-success btn-sm"
          onClick={() => setFormData(!formData)}
        >
          <i className="fa fa-plus"></i> {formData ? "Close Form" : "Add Upcoming Service Commitment"}
        </button>
      </div>
            ...
            <form onSubmit={handleSubmit} encType="multipart/form-data">
              <div className="row mb-3">
                <div className="col-md-3">
                  ...
                  <div className="col-md-3">
                  <label className="form-label">Icon Text</label>
                  <input
                    type="text"
                    name="icon_text"
                    className="form-control"
...
<div className="col-md-3">
      ...
      {/* ✅ Table */}
      <div className="card">
        <div className="card-header">Bageshwer Upcomming Service Commitments</div>
        <div className="card-body">
          ...
          <div className="modal-dialog modal-lg" style={{ maxWidth: "1200px" }}>
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Edit Upcomming Service Commitments</h5>
                <button
                  type="button"
                  className="btn-close"
...
export default UpcommingServiceCommitments;
