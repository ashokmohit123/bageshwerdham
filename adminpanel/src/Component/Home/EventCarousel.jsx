...
const handleSubmit = async (e) => {
 ...
 if (editingId) {
  const response = await axios.put(
    `${BASE_URL}/api/updateevents/${editingId}`,
    fd,
    {
      headers: {
        apikey: "12345",
  ...
  const updatedEvents = response.data.data; // 👈 this is the updated row object

  setEventsslide((prev) =>
    prev.map((item) =>
      item.id === editingId ? updatedEvents : item
    )
  );
  ...
  setShowEditModal(false);
      ...
      // ✅ Add new
      const res = await axios.post(`${BASE_URL}/api/insertevents`, fd, {
        headers: {
          apikey: "12345",
          "Content-Type": "multipart/form-data",
        },
...
console.log(res.data);
      ...
      setEventsslide((prev) => [
        ...prev,
        { id: res.data.id, ...formData, image_url: res.data.image_url, video_url: res.data.video_url },
      ]);
...
//select all data table
 ...
 const [eventsslide, setEventsslide] = useState([]);
  ...
  // ✅ Fetch data from API
  useEffect(() => {
    axios.get(`${BASE_URL}/api/selectevents`, {
      headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/api/selectevents/${id}`, {
  ...
  const toggleStatus = async (id) => {
    try {
      const res = await axios.patch(
        `${BASE_URL}/api/selectevents/${id}/status`,
        {},
        { headers: { apikey: "12345" } }
      );
  ...
  return (
    <div className="top-banner">
      ...
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h3>Event Carousel</h3>
        <button
          className="btn btn-success btn-sm"
          onClick={() => setFormData(!formData)}
        >
          <i className="fa fa-plus"></i> {formData ? "Close Form" : "Add Event"}
        </button>
      </div>
              ...
              <div className="modal-body">
                <form onSubmit={handleSubmit} encType="multipart/form-data">
                  <div className="row mb-3">
                    ...
                    <div className="col">
                      <label className="form-label">Change Banner</label>
                      <input
                        type="file"
                        name="image_url"
                        className="form-control"
