  ...
  try {
    ...
    if (editingId) {
      ...
      const res = await axios.put(
        `${BASE_URL}/api/updatebecomeevents/${editingId}`,
        fd,
      ...
      // ✅ Add new
      const res = await axios.post(`${BASE_URL}/api/insertbecomeevents`, fd, {
        headers: { apikey: "12345", "Content-Type": "multipart/form-data" },
      });
...
//select all data table
 ...
 const [eventsslide, setEventsslide] = useState([]);
  ...
  // ✅ Fetch data from API
  useEffect(() => {
    axios.get(`${BASE_URL}/api/selectbecomeevents`, {
      headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/api/deletebecomeevents/${id}`, {
  ...
  const toggleStatus = async (id) => {
    try {
      const res = await axios.patch(
        `${BASE_URL}/api/statusbecomeevents/${id}/status`,
        {},
        { headers: { apikey: "12345" } }
      );
  ...
  return (
    <div className="top-banner">
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        >
          <i className="fa fa-plus"></i> {formData ? "Close Form" : "Add Become Event"}
        </button>
      </div>
