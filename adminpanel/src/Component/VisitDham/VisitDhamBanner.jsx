...
const VisitDhamBanner = ({ BASE_URL }) => {
  // ✅ Table data
...
const apiUrl = `${BASE_URL}/api/insertvisitdham_banner`;
    console.log('Making request to:', apiUrl); // Log the URL
    if (editingId) {
      // ✅ Update existing
      const res = await axios.put(
        `${BASE_URL}/api/updatevisitdham_banner/${editingId}`,
        fd,
      ...
      // ✅ Add new
      const res = await axios.post(`${BASE_URL}/api/insertvisitdham_banner`, fd, {
        headers: { apikey: "12345", "Content-Type": "multipart/form-data" },
      });
...
//select all data table
 ...
 const [eventsslide, setEventsslide] = useState([]);
  ...
  // ✅ Fetch data from API
  useEffect(() => {
    axios.get(`${BASE_URL}/api/selectvisitdham_banner`, {
      headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/api/deletevisitdham_banner/${id}`, {
  ...
  const toggleStatus = async (id) => {
    try {
      const res = await axios.patch(
        `${BASE_URL}/api/statusvisitdham_banner/${id}/status`,
        {},
        { headers: { apikey: "12345" } }
      );
  ...
  return (
    <div className="top-banner">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h3>Visit Dham Banner</h3>
        <button
          className="btn btn-success btn-sm"
          onClick={() => setFormData(!formData)}
        >
          <i className="fa fa-plus"></i> {formData ? "Close Form" : "Add Visit Dham Banner"}
        </button>
      </div>
      ...
      {/* ✅ Table */}
      <div className="card">
        <div className="card-header">Bageshwer visit dham banner List</div>
        <div className="card-body">
          ...
          <div className="modal-dialog modal-lg" style={{ maxWidth: "1200px" }}>
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Edit Visit Dham Banner</h5>
                <button
                  type="button"
                  className="btn-close"
...
export default VisitDhamBanner;
