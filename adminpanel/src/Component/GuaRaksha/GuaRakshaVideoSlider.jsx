...
const GuaRakshaVideoSlider = ({ BASE_URL }) => {
  ...
  try {
    ...
    if (editingId) {
      const res = await axios.put(
        `${BASE_URL}/api/updategauraksha_videoslider/${editingId}`,
        fd,
        {
      ...
      const res = await axios.post(
        `${BASE_URL}/api/insertgauraksha_videoslider`,
        fd,
        { headers: { apikey: "12345", "Content-Type": "multipart/form-data" } }
      );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/api/deletegauraksha_videoslider/${id}`, {
  ...
  const toggleStatus = async (id) => {
    try {
      const res = await axios.patch(
        `${BASE_URL}/api/statusgauraksha_videoslider/${id}/status`,
        {},
        { headers: { apikey: "12345" } }
      );
      setEventsslide(prev =>
  ...
  return (
    <div className="top-banner">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h3>Gau Raksha Video Slider</h3>
        <button
          className="btn btn-success btn-sm"
          onClick={() => setShowForm(!showForm)}
        >
          <i className="fa fa-plus"></i> {showForm ? "Close Form" : "Add Gau Raksha Video"}
        </button>
      </div>
